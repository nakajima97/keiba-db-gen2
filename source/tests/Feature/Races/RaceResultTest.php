...
test('valid payout text is stored with 8 race_payouts records', function () use ($sampleText, $resultSampleText) {
    ...
    $response = $this->actingAs($user)->post(route('races.result.store', ['uid' => $raceUid]), [
        'result_text' => $resultSampleText, 'text' => $sampleText,
    ]);
...
test('JRA format payout text (ticket type on separate header line) is stored correctly', function () use ($jraSampleText, $resultSampleText) {
    ...
    // Act
    $response = $this->actingAs($user)->post(route('races.result.store', ['uid' => $raceUid]), [
        'result_text' => $resultSampleText, 'text' => $jraSampleText,
    ]);
...
test('single: ヒットした馬番の payout_amount が更新される', function () use ($sampleText, $resultSampleText) {
    // Arrange
    $user = User::factory()->create();
    ['venueId' => $venueId, 'now' => $now] = createRaceResultMasterData();
    ['raceId' => $raceId, 'raceUid' => $raceUid] = createRaceWithUid($venueId, $now);
...
test('box: ヒットした組み合わせの payout_amount が更新される', function () use ($sampleText, $resultSampleText) {
    // Arrange
    $user = User::factory()->create();
    ['venueId' => $venueId, 'now' => $now] = createRaceResultMasterData();
    ['raceId' => $raceId, 'raceUid' => $raceUid] = createRaceWithUid($venueId, $now);
...
test('nagashi: ヒットした組み合わせの payout_amount が更新される', function () use ($sampleText, $resultSampleText) {
    // Arrange
    $user = User::factory()->create();
    ['venueId' => $venueId, 'now' => $now] = createRaceResultMasterData();
    ['raceId' => $raceId, 'raceUid' => $raceUid] = createRaceWithUid($venueId, $now);
...
test('formation: ヒットした組み合わせの payout_amount が更新される', function () use ($sampleText, $resultSampleText) {
    // Arrange
    $user = User::factory()->create();
    ['venueId' => $venueId, 'now' => $now] = createRaceResultMasterData();
    ['raceId' => $raceId, 'raceUid' => $raceUid] = createRaceWithUid($venueId, $now);
...
test('umatan: 着順が一致した場合はヒット、逆順の場合はヒットしない', function () use ($sampleText, $resultSampleText) {
    // Arrange
    $user = User::factory()->create();
    ['venueId' => $venueId, 'now' => $now] = createRaceResultMasterData();
    ['raceId' => $raceId, 'raceUid' => $raceUid] = createRaceWithUid($venueId, $now);
...
test('sanrentan: 着順が一致した場合はヒット、着順が異なる場合はヒットしない', function () use ($sampleText, $resultSampleText) {
    // Arrange
    $user = User::factory()->create();
    ['venueId' => $venueId, 'now' => $now] = createRaceResultMasterData();
    ['raceId' => $raceId, 'raceUid' => $raceUid] = createRaceWithUid($venueId, $now);
...
test('購入した組み合わせがレース結果に含まれない場合、payout_amount は null のまま', function () use ($sampleText, $resultSampleText) {
    // Arrange
    $user = User::factory()->create();
    ['venueId' => $venueId, 'now' => $now] = createRaceResultMasterData();
    ['raceId' => $raceId, 'raceUid' => $raceUid] = createRaceWithUid($venueId, $now);
...
test('wide で複数組み合わせがヒットした場合、payout_amount は合算される', function () use ($sampleText, $resultSampleText) {
    // Arrange
    $user = User::factory()->create();
    ['venueId' => $venueId, 'now' => $now] = createRaceResultMasterData();
    ['raceId' => $raceId, 'raceUid' => $raceUid] = createRaceWithUid($venueId, $now);
...
test('同じレースの別ユーザーの馬券には影響しない', function () use ($sampleText, $resultSampleText) {
    // Arrange
    $user = User::factory()->create();
...
test('別レースの馬券には影響しない', function () use ($sampleText, $resultSampleText) {
    // Arrange
    $user = User::factory()->create();
    ['venueId' => $venueId, 'now' => $now] = createRaceResultMasterData();
    ['raceId' => $raceId, 'raceUid' => $raceUid] = createRaceWithUid($venueId, $now);
...
test('unit_stake が 100円より多い場合、払い戻し金額は購入金額に比例してスケールされる', function () use ($sampleText, $resultSampleText) {
    // Arrange
    $user = User::factory()->create();
    ['venueId' => $venueId, 'now' => $now] = createRaceResultMasterData();
    ['raceId' => $raceId, 'raceUid' => $raceUid] = createRaceWithUid($venueId, $now);
