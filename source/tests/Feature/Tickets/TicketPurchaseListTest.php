...
function createTicketPurchase(int $userId, int $raceId, int $ticketTypeId, int $buyTypeId, CarbonInterface $now, int $unitStake = 100): void
{
...
test('nagashi（流し）・2点: purchase_amount が 単価×2 で返される / num_combinations が 2 で返される', function () {
    // Arrange
    $user = User::factory()->create();
    ...
    $response->assertInertia(fn (Assert $page) => $page
        ->component('tickets/index')
        ->where('purchases', function ($purchases) {
            expect($purchases[0]['purchase_amount'])->toBe(200);
...
test('box（ボックス）・3点: purchase_amount が 単価×3 で返される', function () {
    // Arrange
    $user = User::factory()->create();
    ...
    $response->assertInertia(fn (Assert $page) => $page
        ->component('tickets/index')
        ->where('purchases', function ($purchases) {
            expect($purchases[0]['purchase_amount'])->toBe(300);
...
test('formation（フォーメーション）・重複なし: purchase_amount が 単価×全点数 で返される', function () {
    // Arrange
    $user = User::factory()->create();
    ...
    $response->assertInertia(fn (Assert $page) => $page
        ->component('tickets/index')
        ->where('purchases', function ($purchases) {
            expect($purchases[0]['purchase_amount'])->toBe(800);
...
test('formation（フォーメーション）・重複馬番あり: 有効点数×単価 で返される', function () {
    ...
    $response->assertInertia(fn (Assert $page) => $page
        ->component('tickets/index')
        ->where('purchases', function ($purchases) {
            expect($purchases[0]['purchase_amount'])->toBe(3600);
...
test('unit_stake が null の場合: purchase_amount が null で返される', function () {
    // Arrange
    $user = User::factory()->create();
    ...
    $response->assertInertia(fn (Assert $page) => $page
        ->component('tickets/index')
        ->where('purchases', function ($purchases) {
            expect($purchases[0]['purchase_amount'])->toBeNull();
...
test('num_combinations: unit_stake が null でも num_combinations が返される', function () {
    // Arrange
    $user = User::factory()->create();
