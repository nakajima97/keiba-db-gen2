...
test('nagashi（流し）・2点: unit_stake が 単価×2 で返される / num_combinations が 2 で返される', function () {
    // Arrange
    $user = User::factory()->create();
    ...
    $response->assertInertia(fn (Assert $page) => $page
        ->component('tickets/index')
        ->where('purchases', function ($purchases) {
            expect($purchases[0]['unit_stake'])->toBe(200);
...
test('box（ボックス）・3点: unit_stake が 単価×3 で返される', function () {
    // Arrange
    $user = User::factory()->create();
    ...
    $response->assertInertia(fn (Assert $page) => $page
        ->component('tickets/index')
        ->where('purchases', function ($purchases) {
            expect($purchases[0]['unit_stake'])->toBe(300);
...
test('formation（フォーメーション）・重複なし: unit_stake が 単価×全点数 で返される', function () {
    // Arrange
    $user = User::factory()->create();
    ...
    $response->assertInertia(fn (Assert $page) => $page
        ->component('tickets/index')
        ->where('purchases', function ($purchases) {
            expect($purchases[0]['unit_stake'])->toBe(800);
...
test('formation（フォーメーション）・重複馬番あり: 有効点数×単価 で返される', function () {
    ...
    $response->assertInertia(fn (Assert $page) => $page
        ->component('tickets/index')
        ->where('purchases', function ($purchases) {
            expect($purchases[0]['unit_stake'])->toBe(3600);
...
test('unit_stake が null の場合: unit_stake が null で返される', function () {
    // Arrange
    $user = User::factory()->create();
    ...
    $response->assertInertia(fn (Assert $page) => $page
        ->component('tickets/index')
        ->where('purchases', function ($purchases) {
            expect($purchases[0]['unit_stake'])->toBeNull();
