 ...
 * } $params
 */
function createTicketPurchase(array $params): void
{
    $now = now();
...
test('購入記録がある場合 summary の各値が正しく計算されて返る', function () {
    ...
    $response->assertInertia(fn ($page) => $page
        ...
        ->where('summary.total_return_rate', 150.0)
    );
});
...
test('available_years に購入記録が存在する年がすべて含まれる', function () {
    ...
    $response->assertInertia(fn ($page) => $page
        ...
        ->where('available_years', fn ($years) => in_array(2025, $years) && in_array(2026, $years))
    );
});
